...
if (isset($_POST['add_movimento'])) {
    ...
    $esmalte_escolhida = trim($_POST['esmalte_id'] ?? '');
    ...
    $erros = array();
    if (!ctype_digit((string)$esmalte_escolhida)) { $erros[] = 'esmalte inválida.'; }
    ...
    if (empty($erros)) {
        ...
        if ($tipo_movimento === 'saida') {
            $esmalte_id = (int)$esmalte_escolhida;
            $qtd = (int)$quantidade_movimento;
            $entradas = $conn->query("SELECT COALESCE(SUM(quantidade),0) AS total FROM movimentacoes WHERE esmalte_id = $pizza_id AND tipo = 'entrada'")->fetch_assoc()['total'];
            $saidas = $conn->query("SELECT COALESCE(SUM(quantidade),0) AS total FROM movimentacoes WHERE esmalte_id = $pizza_id AND tipo = 'saida'")->fetch_assoc()['total'];
    ...
    if (empty($erros)) {
        $esmalte_id = (int)$esmalte_escolhida;
        $qtd = (int)$quantidade_movimento;
        $stmt = $conn->prepare("INSERT INTO movimentacoes (esmalte_id, usuario_id, data_hora, tipo, quantidade, observacoes) VALUES (?, ?, NOW(), ?, ?, ?)");
        if ($stmt) {
            $stmt->bind_param('iisis', $esmalte_id, $usuario_id, $tipo_movimento, $qtd, $observacoes_movimento);
            if ($stmt->execute()) {
...
$sql_movimentacoes = "SELECT m.*, p.nome as esmalte_nome, u.nome as usuarioesmaltes
...
<head>
    ...
    <title>Movimentações - Studio D.I.Y</title>
                    ...
                    <div class="form-group">
                        ...
                        <select name="pizza_id" required>
                            ...
                            <?php while($esmalte = $resultado_esmaltes->fetch_assoc()): 
                                ...
                                $sql_entradas = "SELECT SUM(quantidade) as total FROM movimentacoes WHERE esmalte_id = $pizza_id AND tipo = 'entrada'";
                                $entradas = $conn->query($sql_entradas)->fetch_assoc();
                                $total_entradas = $entradas['total'] ? $entradas['total'] : 0;
                                
                                $sql_saidas = "SELECT SUM(quantidade) as total FROM movimentacoes WHERE esmalte_id = $pizza_id AND tipo = 'saida'";
